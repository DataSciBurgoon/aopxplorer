#create an AOP network visualization using D3 ----------------------------------
#'
#' Visualizes an AOP from AOPXplorer using D3 and shiny.
#'
#' Based on networkD3 package. I added functionality for directionality into
#' the JS code, and changed function names here to prevent clashing.
#'
...
#'
#' # Create graph
#' aopxplorer(Links = MisJson$links, Nodes = MisJson$nodes, Source = "source",
#'              Target = "target", Value = "value", NodeID = "name",
...
#' @rdname aopxplorer-shiny
#' @export
aopOutput <- function(outputId, width = "100%", height = "500px") {
  shinyWidgetOutput(outputId, "aopxplorer", width, height, package="aopxplorer")
}

#' @rdname aopxplorer-shiny
#' @export
renderAop <- function(expr, env = parent.frame(), quoted = FALSE) {
  if (!quoted) { expr <- substitute(expr) } # force quoted
  shinyRenderWidget(expr, aopOutput, env, quoted = TRUE)
}


#' Shiny bindings for aopxplorer widgets
#'
...
#' @param expr An expression that generates a aopxplorer graph
